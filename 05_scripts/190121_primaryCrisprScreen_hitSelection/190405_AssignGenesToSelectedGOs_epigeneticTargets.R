...
#Go terms
GOs = read.table("03_extractedData/190121_primaryCrisprScreen_hitSelection/GOenrichment_epiLib_manuallyCurated.txt", header = T, sep = "\t")
...
for(i in 1:nGO) {
  ...
  #Eliminate those genes from the epiTargets file
  tmp = epiTargets %>%
    filter(!gene %in% genesForOutput$gene)
  
  #replace epiTargets with tmp
  epiTargets = tmp
...
epiTargets$ID = "other"
epiTargets$Description = "other"

outputFile = bind_rows(outputFile, epiTargets)

setwd("03_extractedData/190121_primaryCrisprScreen_hitSelection/")
write.table(outputFile, "epigeneticTargets_assignedToGOs.txt", sep = "\t", col.names = T, row.names = FALSE, quote = FALSE)
