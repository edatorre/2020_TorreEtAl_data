...
#Load plate map
plateMap = as.tibble(read.table("02_metadata/181101_ColonyGrowth36_validationOfKOs_reanalyzed/plateMaps/plate2.txt", header = T))

#Read in baseline plate 
baselineData = as_tibble(read.csv("01_rawData/181101_ColonyGrowth36_validationOfKOs_reanalyzed/summarizedResults/row2_baseline_plateSummary.csv"))
baselineData = baselineData[,1:2]
colnames(baselineData) = c("well", "cellNumber_baseline")

#Read in colony growth data
colonyGrowthData = as_tibble(read.csv("01_rawData/181101_ColonyGrowth36_validationOfKOs_reanalyzed/summarizedResults/row2_onPLX4032_plateSummary.csv"))
...
write.table(mergedData, file ="plate2_extractedData.txt", row.names = FALSE, col.names = TRUE, sep = "\t", quote = FALSE)
