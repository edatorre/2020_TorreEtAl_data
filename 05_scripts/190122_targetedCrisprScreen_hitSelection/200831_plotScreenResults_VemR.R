...
primaryScreen = read.table("03_extractedData/190121_primaryCrisprScreen_hitSelection/VemRscreen_medianlFC_targetedScreenGuides.txt", header = T)
...
#Load results
cellLine1 = read.table("03_extractedData/190122_targetedCrisprScreen_hitSelection/VemRanalysis_WM989.bulk_allInfo.txt", header = T)
cellLine1 = cellLine1 %>%
...
#Load results
cellLine2 = read.table("03_extractedData/190122_targetedCrisprScreen_hitSelection/VemRanalysis_X451Lu_allInfo.txt", header = T)
cellLine2 = cellLine2 %>%
  dplyr::select(target, lFC, medianlFC, sdlFC) %>%
  distinct()
...
# #Make plottin tables 
# plot_screenResults = ggplot(data = screenResults, aes(x = plottingID, y = medianlFC, ymin = (medianlFC - sdlFC), ymax = (medianlFC + sdlFC),  fill = cellLine)) +
#   geom_bar(stat = "identity", position = "dodge") +
#   geom_errorbar(position = "dodge", size = 0.25, alpha = 0.5) +
#   geom_text(aes(x = plottingID, y = -3, label = target, color = EffectOnScreen_VemR), size = 2) +
#   scale_color_manual(values = c("down" = "#808080", "up" = "#00CC00", "NA" = "blue")) +
#   geom_hline(yintercept = c(0), size = 0.25) + 
#   facet_grid(. ~ VemRtier, scales = "free") +
#   coord_flip() +
#   theme_classic()
# plot_screenResults
# ggsave("04_plots/190122_targetedCrisprScreen_hitSelection/VemRscreen_twoCellLines.PDF", height = 8, width = 7)
...
plot_screenResults = ggplot(data = screenResults, aes(x = plottingID, y = medianlFC)) +
  ...
  theme_classic()
plot_screenResults
ggsave("04_plots/190122_targetedCrisprScreen_hitSelection/200831_VemRscreen_twoCellLines.PDF", height = 8, width = 7)
...
plot_screenResults = ggplot(data = screenResults, aes(x = plottingID, y = medianlFC)) +
  ...
  theme_classic()
plot_screenResults
ggsave("04_plots/190122_targetedCrisprScreen_hitSelection/200831_VemRscreen_twoCellLines_dotsOnly.PDF", height = 8, width = 7)
